...
async function debugWarAlerts() {
  ...
  try {
    ...
    for (const channel of warChannels) {
      console.log(`  • Channel: ${channel.channelId} (${channel.channelName})`);
      console.log(`    Server: ${channel.discordServer.serverName}`);
      console.log(`    Alliance: ${channel.discordServer.alliance?.name || 'No alliance'}`);
    }
